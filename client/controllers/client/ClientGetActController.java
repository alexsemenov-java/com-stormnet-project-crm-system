...
public class ClientGetActController implements Controller, Initializable {
    @FXML
    ...
    public void saveBtnPressed() throws IOException {
        if (ChooseManager.getValue() == null || OrderTime.getValue() == null || ChooseDate.getValue() == null) {
        ...
        act.setClientFirstName(client.getFirstName());
        act.setClientLastName(client.getLastName());
        act.setManagerId(ChooseManager.getValue().getId());
        act.setManagerLastName(ChooseManager.getValue().getLastName());
        act.setManagerFirstName(ChooseManager.getValue().getFirstName());
        act.setDate(ChooseDate.getValue());
        act.setTime(OrderTime.getValue().toString());
        act.setOffice(ChooseManager.getValue().getOffice());
        act.setClientComment(clientComment.getText());
        act.setPhoneNumber(client.getPhoneNumber());

        act.setRating("");
        act.setManagerComment("");

        ActService actService = ServiceFactory.getServiceFactory().getActService();
        actService.save(act);

        AppWindows appWindows = AppWindows.getInstance();
        appWindows.hideWindow(WindowConfigs.ClientGetActWindow);
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        try {
            allPersons = managerService.loadAll();
        } catch (IOException e) {
            e.printStackTrace();
        }
